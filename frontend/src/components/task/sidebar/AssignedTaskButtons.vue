<template>
  <div class="task-button">
    ...
    >
      ...
      ><span class="assign-task">Assign Tasks</span>
    </button>
    <button
      type="button"
      id="unassignTaskButton"
      class="btn btn-primary btn-info btn-create"
      :class="{ disabled: isDisabled === true }"
      @click="unassignTask"
    >
      <i class="fa fas fa-tasks"></i><b class="margin-b"></b
      ><span class="assign-task">Unassign Tasks</span>
    </button>
  </div>
  <div>
...
const assignTask = async () => {
  try {
    ...
    const response = await fetch(
      `http://localhost:5000/api/drones/${selectedDroneId.value}/tasks/bulk_assign`,
      {
        headers: {
    ...
    toastRef.value.showMessage({
      messages: `Tasks are assigned to drone with drone id ${selectedDroneId.value}`,
      type: 'success'
    })
...
const unassignTask = async () => {
  try {
    const csrfToken = await getCsrfToken()
    const response = await fetch(
      `http://localhost:5000/api/drones/${selectedDroneId.value}/tasks/bulk_cancel_assignment`,
      {
        headers: {
          'Access-Control-Allow-Credentials': true,
          'X-CSRFToken': csrfToken
    ...
    toastRef.value.showMessage({
      messages: `Tasks are unassigned to drone with drone id ${selectedDroneId.value}`,
      type: 'success'
    })
    socketStore.filterAssignedTasks(selectedDroneId.value)
  } catch (error) {
