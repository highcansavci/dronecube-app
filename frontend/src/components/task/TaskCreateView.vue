...
const createTask = async () => {
  try {
    ...
    formData.append('start_date', new Date(task.start_date).toISOString())
    formData.append('end_date', new Date(task.end_date).toISOString())
    ...
    toastRef.value.showMessage({
      messages: `${data.title} task is successfully created`,
      type: 'success'
    })
