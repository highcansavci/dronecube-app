      ...
      <div class="create-task">
        ...
        <form id="regForm" class="requires-validation" novalidate>
          <div class="form-group">
            <h4>Task Title</h4>
            <input class="form-control" type="text" name="name" v-model="task.title" />
          </div>
          <div class="form-group">
            <h4>Task Position</h4>
...
const { isOpenProp, selectedTaskId } = toRefs(props)
...
const updateTask = async () => {
  try {
    ...
    formData.append(
      'start_date',
      task.start_date === null ? task.start_date : new Date(task.start_date).toISOString()
    )
    formData.append(
      'end_date',
      task.end_date === null ? task.end_date : new Date(task.end_date).toISOString()
    )
    ...
    toastRef.value.showMessage({
      messages: `${data.title} task is successfully updated`,
      type: 'success'
    })
